...
find "src/" -type f -name "*.cu" | while read -r cu_file; do
    ...
    nvcc -O3 -std=c++17 -Xcompiler -fPIC -c $cu_file -o $o_file
done

find "src/" -type f -name "*.cpp" | while read -r cpp_file; do
    o_file="$(basename $cpp_file)"
    o_file="build/${o_file%.cpp}.o"
    nvcc -O3 -std=c++17 -Xcompiler -fPIC -I/workspace/submodules/CudaSharedPtr -c $cpp_file -Xcompiler $(python3 -m pybind11 --includes) -o $o_file
