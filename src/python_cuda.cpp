...
PYBIND11_MODULE(python_cuda, m) {
    pybind11::class_<Vector>(m, "Vector", "1 dimensional vector of floats with CUDA support.")
        ...
        .def(pybind11::init<pybind11::array_t<float> &, bool>(), "A vector initialized from numpy array `array`. By default, it will use the same memory for the device-side array as the input array. You can avoid it by setting `copy=True`.", pybind11::arg("array"), pybind11::arg("copy") = false)
        .def("get_array", &Vector::getArray, "Returns the data as numpy.ndarray. Note that it might be out-of-sync with the actual, GPU array if you modified that.")
